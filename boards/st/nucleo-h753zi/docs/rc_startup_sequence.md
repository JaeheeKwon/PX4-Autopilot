...
| Sensor presence | `SYS_HAS_MAG=0`, `SYS_HAS_BARO=1`, `SYS_HAS_GPS=1` | Correct if jMAVSim or another HITL simulator supplies GPS and baro. |
| EKF aiding | `EKF2_GPS_CTRL=7`, `EKF2_HGT_REF=1`, `EKF2_BARO_CTRL=1`, `EKF2_MAG_TYPE=5`, `EKF2_EV_CTRL=0` | Consistent with no real magnetometer and HIL GPS/baro aiding. |
...
| USB MAVLink | `SYS_USB_AUTO=2`, `USB_MAV_MODE=2` | Correct for USB CDC MAVLink HITL/QGC workflow. |
...
|---:|---|---|
| 1 | Keep the active `rc.board_defaults` as-is for HITL. | It matches the no-sensor board and fixes the arming workflow. |
